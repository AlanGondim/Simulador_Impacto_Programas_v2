...
with st.container(border=True):
    ...
    receita_liq = d2.number_input("Receita líquida atual", min_value=1.0, value=1000.0, step=1000.0)
...
if st.sidebar.button("💾 SALVAR DADOS E GERAR PDF"):
    ...
    try:
        ...
        ax1.bar(['Baseline', 'Projetado'], [margem_atual, margem_final], color=['#003366', '#d32f2f'])
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix=".png") as tmp2:
            ...
            pdf.image(tmp2.name, x=110, y=y_graficos, w=80)
        plt.close(fig_radar)
        pdf.set_y(-65)
